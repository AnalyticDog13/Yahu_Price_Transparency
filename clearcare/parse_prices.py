...
by CPT code, and writes a normalized CSV and SQLite database.
...
# CPT code → broad category (shown in UI dropdown)
...
# Hospital registry — add new hospitals here
HOSPITAL_FILES = [
    {
...
def parse_hospital_csv(hospital: dict) -> list[dict]:
    ...
    with open(path, encoding="utf-8-sig", errors="replace") as f:
        ...
        for i, row in enumerate(reader):
            ...
            payer = row.get("payer_name", "").strip().upper()
            payer = re.sub(r"\s+", " ", payer)
...
def write_csv(rows: list[dict]):
    ...
    print(f"CSV written  → {OUT_CSV}  ({os.path.getsize(OUT_CSV) / 1024:.0f} KB, {len(rows):,} rows)")
...
def main():
    ...
    by_hospital = Counter(r["hospital_name"] for r in all_rows)
    by_category = Counter(r["procedure_category"] for r in all_rows)
    for hosp, n in sorted(by_hospital.items()):
        print(f"  {hosp}: {n:,} rows")
    print()
